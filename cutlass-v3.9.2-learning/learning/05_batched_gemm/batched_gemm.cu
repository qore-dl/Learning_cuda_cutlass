...
这个例子演示了如何使用cutlass以两种不同的方式来计算批处理的strided gemm 算法：
  1. By specifying pointers to the first matrices of the batch and the stride 
  between the consecutive matrices of the batch (this is called a strided batched gemm).
    strided batched gemm: 通过指定指向批处理的第一个矩阵的指针和批处理的连续矩阵之间的跨距（这称为跨距批处理gem）。
  2. By copying pointers to all matrices of the batch to the device memory (this is called an array gemm).
  array gemm: 通过将指向批处理中所有矩阵的指针复制到设备内存（这称为数组gem）
    ...
    for(int batch_idx=0;batch_idx<batch_count;batch_idx++){
        for(int n_idx=0;n_idx<n;n_idx++){
            for(int m_idx=0;m_idx<m;m_idx++){
                T accum = beta*C[batch_idx*batch_stride_C+ldc*n_idx+m_idx];
                for(int k_idx=0;k_idx<k;k_idx++){
                    accum += alpha * A[batch_idx * batch_stride_A + k_idx * lda + m_idx] * B[batch_idx * batch_stride_B + n_idx * ldb + k_idx];
                }
                C[batch_idx*batch_stride_C+ldc*n_idx+m_idx] = accum;
            }
...
cudaError_t run_batched_gemm(bool use_array){
    ...
    int const lda = m;
    int const ldb = batch_count * k;
    int const ldc = m;

    int const count_A = lda * k * batch_count;
    int const count_B = ldb * n;
    int const count_C = ldc * n * batch_count;
    ...
    result = cudaMalloc(&A,sizeof(float)*count_A);
    if (result != cudaSuccess) {
        std::cerr << "cudaMalloc result = " << result << std::endl;
        return result;
    }

    result = cudaMalloc(&B,sizeof(float)*count_B);
    if (result != cudaSuccess) {
        std::cerr << "cudaMalloc result = " << result << std::endl;
        return result;
    }

    result = cudaMalloc(&C,sizeof(float)*count_C);
    if (result != cudaSuccess) {
        std::cerr << "cudaMalloc result = " << result << std::endl;
        return result;
    }
    ...
    if(use_array){
        ...
        /*
        对batch元素进行(按照变换地址顺序重新排列)排列，以强调GemmArray不依赖于由固定步幅分隔的矩阵
        */
        ...
        result = cutlass_strided_batched_sgemm(
            m,n,k,alpha,A,lda,batch_stride_A,B,ldb,batch_stride_B,C,ldc,batch_stride_C,beta,batch_count);
        if (result != cudaSuccess){
            return result;
        }
    }
    ...
    result = strided_batched_gemm_nn_reference(m,n,k,alpha,ref_A,lda,batch_stride_A,
        ref_B,ldb,batch_stride_B,ref_C,ldc,batch_stride_C,beta,batch_count);
    if (result != 0){
        return result;
    }
