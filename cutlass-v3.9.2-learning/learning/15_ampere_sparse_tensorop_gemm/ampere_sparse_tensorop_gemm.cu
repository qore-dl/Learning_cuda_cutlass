/**
Please check example 07, 08 and 17 for the basics of dense tensor op gemm kernels.  
NVIDIA Ampere architecture also supports structured sparse tensor op for tf32, fp16, int8 and int4.
请查看例07、08和17，了解密集张量算子核的基础知识。
NVIDIA Ampere架构还支持tf32、fp16、int8和int4的结构化稀疏张量运算。

Sparse GEMM kernels needs to takes an additional E matrix which stores the meta data.  
The format of meta data is different for every data types. 
CUTLASS templates can automatically infer it based on input A and B.  Check code below.

稀疏GEMM核需要额外的E矩阵来存储元数据。
元数据的格式对于每种数据类型都是不同的。
CUTLASS模板可以根据输入A和B自动推断。

Moreover, matrix E needs to be preprocessed so that it can use ldmatrix to load into the registers
 efficiently.

此外，矩阵E需要预处理，以便它可以使用ldmatrix高效地加载到寄存器中。
*/
