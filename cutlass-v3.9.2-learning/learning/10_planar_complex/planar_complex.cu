/*! \file
  ...
  This example demonstrates the CUTLASS Library's exposure of planar complex GEMM kernels supporting the batched strided mode.
  这个例子演示了CUTLASS库对支持batched strided 模式的平面复数GEMM内核的揭示。

  These kernels represent complex matrices by storing the real and imaginary parts of 
  the matrix in disjoint regions in memory.
  These real-valued matrices are stored using existing cuBLAS layouts 
  as either column-major or row-major layouts with a single leading dimension 
  indicating the stride between columns or rows.

  这些核通过将矩阵的实部和虚部存储在内存的不相交区域中来表示复数矩阵。
  这些实值矩阵使用现有的cuBLAS布局作为主要列布局或主要行布局存储，并使用单个导向维度表示列或行之间的跨距。

  The CUTLASS Library collects multiple template instantiations in a data structure and 
  offers a BLAS-like dispatch API to invoke the appropriate kernel on the Volta or Turing architectures.
  CUTLASS库在数据结构中收集多个模板实例化，并提供类似blas的分派API来调用Volta或Turing架构上的适当内核。

  CUTLASS decouples matrix layout from complex transformation, 
  so four possible transformations are possible on the A and B operands:

  CUTLASS将矩阵布局与复变换解耦，
  所以A和B操作数有四种可能的变换：

    n:  column-major
    c:  column-major complex conjugate （共轭）
    t:  row-major
    h:  row-major complex conjugate （共轭）

  The CUTLASS Library contains many kernel instances specialized for architecture, data type, tile size, and alignment. 
  This can result in long compile times.
  CUTLASS库包含许多专门用于体系结构、数据类型、tile大小和对齐的内核实例。这可能导致较长的编译时间。

  To build strictly the planar complex kernels needed for general application, execute the following CMake command in an empty build directory.
  要严格构建一般应用程序所需的平面复数内核，请在空构建目录中执行以下CMake命令。  
  ...
  这将为Volta和Turing架构构建所有平面复数GEMM变体。
  ...
  若要严格构建此示例所需的内核，可以指定更窄的筛选字符串，如下所示。
  这仅构建针对“CN”布局配置（A 操作数共轭，A 和 B 均为列主序）的平面复数 GEMM 以利用张量核心。
...
struct Options {
    ...
    double gflops(double runtime_s) const{
        ...
        // Number of real-valued mulitply-adds
...
///////////////////////////////////////////////////////////////////////////////////////////////////
/*
集群信息：多少张卡、模型、模型的配置、有哪些负载、有哪些分析工具
指标体系：应用级、模型级、op级
探测发现：insight，case （分析结果组织，三个层面，给出结果呈现和结论）
    1. 同种模型在不同硬件上性能的差异（吞吐量）
    2. 集群整体资源利用率
    3. 模型不同配置在同种硬件上运行的性能差异（主要配置对性能的影响： 参数规模、并行度、precision）
    4. 任务配额：卡数，分布的情况
    5. 流量负载的指标

集群资源配置、部署了多少服务、服务的负载
...
public:
    ...
    void initialize(){
        ...
        cutlass::reference::device::BlockFillRandomUniform(
            tensor_A.get(),tensor_A.size(),seed,ElementA(scope_max),ElementB(scope_min),0);
    ...
    Result profile(Options const &options){
        ...
        ElementA *ptrA = tensor_A.get();
        ElementA *ptrB = tensor_B.get();
        ElementC *ptrC = tensor_C.get();
        ElementC *ptrD = tensor_D.get();
        ...
        typename LayoutC::Stride::Index ldd = LayoutD::packed({problem_size.m(),problem_size.n()}).stride(0);
